...
function [sys,x0,str,ts,simStateCompliance] = encoder_a_rpm(t,x,u,flag, Ts, Tm, ppv_encoder)
...
switch flag,
  ...
  %%%%%%%%%%%%%%%%%%
  case 0,
    [sys,x0,str,ts,simStateCompliance]=mdlInitializeSizes(Ts, Tm, ppv_encoder);
  ...
  %%%%%%%%%%
  case 2,
    sys=mdlUpdate(t,x,u, Ts, Tm, ppv_encoder);

  %%%%%%%%%%%
  % Outputs %
  %%%%%%%%%%%
  case 3,
    sys=mdlOutputs(t,x,u, Ts, Tm, ppv_encoder);
...
%=============================================================================
%
function [sys,x0,str,ts,simStateCompliance]=mdlInitializeSizes(Ts, Tm, ppv_encoder);
...
function sys=mdlUpdate(t,x,u, Ts, Tm, ppv_encoder)
    ...
    if(estado_A_anterior == ESTADO_BAJO) && (tension_A > UMBRAL_TENSION_SUPERIOR)
        % Flanco positivo.
        pulsos_acumulados = pulsos_acumulados + 1;
        estado_A_anterior = ESTADO_ALTO;
    elseif(estado_A_anterior == ESTADO_ALTO) && (tension_A < UMBRAL_TENSION_INFERIOR)
        % Flanco negativo.
        pulsos_acumulados = pulsos_acumulados + 1;
    ...
    sys = [pulsos_acumulados, pulsos_actuales, tiempo_acumulado, estado_A_anterior];
