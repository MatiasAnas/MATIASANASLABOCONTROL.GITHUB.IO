...
const int encoder = 24;
const int nominal_rpm = 6300;
const float freq_motor = nominal_rpm/2*3.14;

auto timer = timer_create_default();
const byte ledPin = 13;
const byte interruptPin = 2;
volatile unsigned int count = 0;
volatile unsigned int showCount = count;
...
void setup() {
  ...
  Serial.begin(9600);
    attachInterrupt(digitalPinToInterrupt(interruptPin), countEdges, FALLING);
}
...
void loop() {
  ...
  delay(1);
  takeReading();
  Serial.println (showCount*2*3.14159*1000/(encoder), DEC);
